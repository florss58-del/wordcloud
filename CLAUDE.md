...
## 배포 약속 (2026-08-19 은아님 지시)

**수정이 끝나면 물어보지 않고 커밋·배포까지 진행한다.** 이 프로젝트는 수업 당일에 바로 쓰는 도구라
확인을 기다리는 사이가 아깝다. 순서는 검증(문법·렌더링) → 커밋 → `git push origin main`(Vercel 자동 배포)
→ **캐시버스터로 서버 응답 확인** → 보고. 검증에서 걸리면 배포하지 않고 먼저 알린다.

## 추후 작업 (TODO)

- [ ] **링크 모드 리허설**: 실제 수업 투입 전에 세션을 만들어 올리기·고치기를 한 번씩 해 본다 (2026-07-12)
- [ ] **시험용 세션 정리**: admin 목록의 “완전 삭제”로 치운다 (강사 로그인이 필요해 은아님만 가능) (2026-08-19)
- [ ] **수업 전 예시 넣기**: 오늘 만든 세션들에 admin의 “수정”으로 예시 줄을 넣는다 (2026-08-19)
...
- ⚠️ **QR은 `<img>`가 아니라 캔버스로만 보여준다**(`css/style.css`의 `!important` 두 줄).
  `qrcode.js`가 만드는 `<img>`에 **브라우저 확장(어도비 계열)이 마우스를 올릴 때 배지를 붙여 화면을 밀어 버린다.**
  이 두 줄을 지우면 그 증상이 돌아온다 (2026-08-19에 원인 찾는 데 네 번 헛짚음)
- **발표는 크롬 F11이 아니라 발표 화면의 “전체화면” 버튼으로 한다.** F11(브라우저 전체화면)은
  마우스를 위로 올리면 주소창·탭 막대가 다시 나타나며 **화면이 아래로 밀린다.**
  Fullscreen API로 들어가면 크롬 UI가 아예 사라져 밀리지 않는다 (2026-08-19에 겪음)
